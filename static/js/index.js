...
const $calibrationCanvas = document.getElementById('calibrationChart');
      ...
      datasets: [
        {
          ...
          borderColor: 'rgba(31,42,31,0.25)',
          borderDash: [6, 6],
          pointRadius: 0,
          borderWidth: 1.5,
        },
        {
          label: `Core (Brier ${brierCore.toFixed(3)})`,
          data: coreXY,
    ...
    options: {
      ...
      scales: {
        x: {
          ...
          min: 0,
          max: 1,
          title: { display: true, text: 'Mean predicted P(Healthy)' },
        ...
        y: {
          min: 0,
          max: 1,
          title: { display: true, text: 'Fraction Healthy (observed)' },
          grid: { color: 'rgba(0,0,0,0.06)' },
          ticks: { color: muted }
        }
      }
...
function openModal(idx) {
  ...
  // Keep modal verdict background consistent with the light theme.
  const bgColor = isBorderline ? '#fff9ea' : (isHealthy ? '#edf8ee' : '#fdecec');
    ...
    <div class="modal-sticky-top">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:20px;">
        <div style="background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:14px 16px;">
          <div style="font-family:var(--mono);font-size:9px;color:var(--muted);letter-spacing:0.12em;margin-bottom:6px;">CORE MODEL · ${(r.core_model.features || []).length} features</div>
        ...
        <div style="background:var(--surface2);border:1px solid var(--border2);border-radius:8px;padding:14px 16px;">
          <div style="font-family:var(--mono);font-size:9px;color:var(--muted);letter-spacing:0.12em;margin-bottom:6px;">ALL-FEATURES MODEL · ${(main.features || []).length} features</div>
